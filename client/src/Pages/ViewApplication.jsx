...
export default function ViewApplication({ jobId }) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {[1,2,3].map((applicant) => (
          <div key={applicant?.id} className="bg-white shadow-md rounded-lg p-6">
            ...
            <p className="text-gray-700 mb-1">Resume: <a href={applicant?.resumeLink} className="text-blue-500" target="_blank" rel="noopener noreferrer">View Resume</a></p>
            <p className="text-gray-500 text-sm">Applied on: {formatDate(applicant?.appliedDate)}</p>
          </div>
        ))}
      </div>
    </div>
